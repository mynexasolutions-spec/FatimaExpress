...
export async function getAllReviewsAdmin() {
  ...
  const { data } = await supabase
    .from("reviews")
    .select("id, rating, review_text, reviewer_name, is_approved, created_at, products ( name, slug )")
...
async function recomputeProductRating(supabase, productId) {
  ...
  const { data: approved } = await supabase.from("reviews").select("rating").eq("product_id", productId).eq("is_approved", true);
  ...
  await supabase
    .from("products")
    .update({ rating: Math.round(average * 10) / 10, reviews_count: count })
    .eq("id", productId);
}
