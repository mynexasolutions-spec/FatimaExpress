...
import { AlertCircle, ArrowRight, Eye, EyeOff, LoaderCircle, Lock, Mail, Phone, User } from "lucide-react";
import AuthShell from "@/components/auth/AuthShell";
import { useAuth } from "@/context/AuthContext";
import { createCustomerAccount } from "@/actions/customerAuth";
...
export default function RegisterPage() {
  ...
  const inputClass =
    "w-full rounded-2xl border border-slate-200 bg-slate-50/60 py-3 pl-11 pr-4 text-base font-normal text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-purple-600 focus:bg-white focus:ring-4 focus:ring-purple-100/60 shadow-xs";

  return (
    <AuthShell
      title="Create your account"
      subtitle="Set up a Fatima Express account to check out faster, view wholesale pricing, and track orders."
      footer={
        <>
          Already registered?{" "}
    ...
    >
      <form onSubmit={submit} className="space-y-3.5">
        <div>
          <label className="mb-1 block text-sm font-bold uppercase tracking-wider text-slate-600">Full name *</label>
          <div className="relative">
            <User size={17} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
            <input required value={form.fullName} onChange={update("fullName")} className={inputClass} placeholder="John Doe" />
          </div>
        </div>

        <div>
          <label className="mb-1 block text-sm font-bold uppercase tracking-wider text-slate-600">Email address *</label>
          <div className="relative">
        ...
        <div>
          <label className="mb-1 block text-sm font-bold uppercase tracking-wider text-slate-600">Phone</label>
          <div className="relative">
        ...
        <div>
          <label className="mb-1 block text-sm font-bold uppercase tracking-wider text-slate-600">Password *</label>
          <div className="relative">
